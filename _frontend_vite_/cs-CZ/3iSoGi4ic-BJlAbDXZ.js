...
fetch()
`,y=F(),g=E,t=p(null);g.id&&(t.value=([f,U]=ee(()=>K("flash/show",{flashId:g.id})),f=await f,U(),f));const v=p(t.value?.title??"New Play"),h=p(t.value?.summary??""),b=p([]),{model:d,def:V}=L({items:[{label:"Veřejný",value:"public"},{label:"Soukromý",value:"private"}],initialValue:t.value?.visibility??"public"}),o=p(t.value?.script??S);function P(n){Q([{text:"Omikuji",action:()=>{o.value=w}},{text:"Shuffle",action:()=>{o.value=T}},{text:"Quiz",action:()=>{o.value=D}},{text:"Timeline viewer",action:()=>{o.value=M}}],n.currentTarget??n.target)}async function R(){if(t.value!=null)x("flash/update",{flashId:t.value.id,title:v.value,summary:h.value,permissions:b.value,script:o.value,visibility:d.value});else{const n=await x("flash/create",{title:v.value,summary:h.value,permissions:b.value,script:o.value,visibility:d.value});y.push("/play/:id/edit",{params:{id:n.id}})}}function A(){t.value==null?j({text:"Please save"}):J(`/play/${t.value.id}`)}async function H(){if(t.value==null)return;const{canceled:n}=await Y({type:"warning",text:(({x})=>("Jste si jistí že chcete smazat \""+x+"\"?"))({x:t.value.title})});n||(await x("flash/delete",{flashId:t.value.id}),y.push("/play"))}const N=_(()=>[]),$=_(()=>[]);return z(()=>({title:t.value?`${"Upravit Play"}: ${t.value.title}`:"Vytvořit Play"})),(n,e)=>{const B=te("PageWithHeader");return q(),k(B,{actions:N.value,tabs:$.value},{footer:s(()=>[a("div",{class:"xg9Cy"},[a("div",re,[a("div",oe,[c(C,{primary:"",onClick:R},{default:s(()=>[e[5]||(e[5]=a("i",{class:"ti ti-check"},null,-1)),i(" "+r("Uložit"),1)]),_:1}),c(C,{onClick:A},{default:s(()=>[e[6]||(e[6]=a("i",{class:"ti ti-eye"},null,-1)),i(" "+r("Zobrazit"),1)]),_:1}),t.value?(q(),k(C,{key:0,danger:"",onClick:H},{default:s(()=>[e[7]||(e[7]=a("i",{class:"ti ti-trash"},null,-1)),i(" "+r("Smazat"),1)]),_:1})):se("",!0)])])],2)]),default:s(()=>[a("div",ae,[a("div",ie,[c(W,{modelValue:v.value,"onUpdate:modelValue":e[0]||(e[0]=u=>v.value=u)},{label:s(()=>[i(r("Titulek"),1)]),_:1},8,["modelValue"]),c(O,{modelValue:I(d),"onUpdate:modelValue":e[1]||(e[1]=u=>le(d)?d.value=u:null),items:I(V)},{label:s(()=>[i(r("Viditelnost"),1)]),caption:s(()=>[i(r("Putting it private means it won't be visible on your profile, but anyone that has the URL can still access it."),1)]),_:1},8,["modelValue","items"]),c(Z,{modelValue:h.value,"onUpdate:modelValue":e[2]||(e[2]=u=>h.value=u),mfmAutocomplete:!0,mfmPreview:!0},{label:s(()=>[i(r("Popis"),1)]),_:1},8,["modelValue"]),c(C,{primary:"",onClick:P},{default:s(()=>[i(r("Vybrat z předvoleb"),1),e[4]||(e[4]=a("i",{class:"ti ti-chevron-down"},null,-1))]),_:1}),c(G,{modelValue:o.value,"onUpdate:modelValue":e[3]||(e[3]=u=>o.value=u),lang:"is"},{label:s(()=>[i(r("Skript"),1)]),_:1},8,["modelValue"])])])]),_:1},8,["actions","tabs"])}}});export{ke as default};
